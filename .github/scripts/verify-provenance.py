#!/usr/bin/env python3
"""Check that every file a formula names was built by the release it claims.
...
Every file the formula names is asked all four, and each must answer yes:
...
from the formula, and the copies asked about are the ones the bump script
downloaded and hashed. A formula naming no release file, or a file that was not
...
URL = re.compile(r'url "https://github\.com/[^/"]+/[^/"]+/releases/download/[^/"]+/([^"/]+)"')
...
def files_named(formula: Path) -> list[str]:
    """The release files a formula points at, in the order it names them."""
    names = URL.findall(formula.read_text())
    if not names:
        raise Refused(f"{formula} names no release file, so there is nothing to verify")
    return names
...
def verify(
    ...
    """Ask gh about every file the formula names, and refuse on any no."""
    run = run or subprocess.run
    names = files_named(formula)
...
def main(argv: list[str] | None = None, run=None) -> int:
    ...
    ap.add_argument("--formula", required=True, type=Path, help="the formula, e.g. jr.rb")
    ap.add_argument(
        "--downloads",
        default=Path("dist"),
        type=Path,
    ...
    ap.add_argument("--tag", required=True, help="the tag the files were built from, e.g. v0.17.0")
    args = ap.parse_args(argv)

    try:
        verified = verify(args.formula, args.downloads, args.repo, args.workflow, args.tag, run)
